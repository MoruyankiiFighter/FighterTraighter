...
std::tuple<Entity*, Entity*> UIFactory::createButton
	...
	Font* font, Vector2D position, Vector2D anchor, double width, double height, double rotation, 
	CallBackOnClick* clickCallback, CallBackOnClick* stopClickCallback, std::string text, int fontSize)
{
	...
	button->addComponent<UITransform>(position, anchor, Vector2D(width, height));
	...
	textEnt->addComponent<UITransform>(position, anchor, Vector2D(width, height));
	...
	std::string valueText, int valueFontSize)
{
	...
	slider->addComponent<UITransform>(position, anchor, Vector2D(width, height));
	...
	text_->addComponent<UITransform>(Vector2D(position.getX(), position.getY() - fontSize - 20), anchor, Vector2D(width, fontSize));
	text_->addComponent<TextComponent>(text, font, fontSize);
	
	Entity* ValueText_ = state->getEntityManager().addEntity();
	ValueText_->addComponent<UITransform>(Vector2D(position.getX() + width + 10, position.getY() - fontSize / 2), anchor, Vector2D(width, fontSize));
